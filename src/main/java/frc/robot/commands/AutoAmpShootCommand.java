...
public class AutoAmpShootCommand extends EntechCommand {
    ...
    public AutoAmpShootCommand(ShooterSubsystem shoot, IntakeSubsystem intake) {
    ...
    public void execute(){
        if (UserPolicy.ampShoot) {
            if((UserPolicy.shootUptoSpeed && UserPolicy.closetoamp) || ampTimer.get() > 2){
